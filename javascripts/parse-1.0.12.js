 ...
 * Version: 1.0.12
 * Built: Fri Jul 20 2012 13:23:09
 * http://parse.com
 *
...
(function (root) {
  ...
  Parse._request = function (route, className, objectId, method, dataObject, options) {
    if (!Parse.applicationId || !Parse.javaScriptKey) {
      throw "You must specify your applicationId and javaScriptKey using " + "Parse.initialize";
    }
    if (route !== "classes" && route !== "users" && route !== "login" && route !== "requestPasswordReset") {
      throw "First argument must be one of classes, users, or login, not '" + route + "'.";
    }
    ...
    add: function (objects) {
      ...
      _.each(objects, function (object) {
        if (self.targetClassName !== object.className) {
          throw "This relation is on objects of class:" + self.targetClassName + " but got object of class:" + object.className;
        }
      });
...
}.call(this)); /*global navigator: false */
(function (root) {
  root.Parse = root.Parse || {};
  var Parse = root.Parse;
  var _ = Parse._;
...
(function (root) {
  ...
  _.extend(Parse.Object.prototype, Parse.Events, /** @lends Parse.Object.prototype */ {
    _existed: false,
    /**
      ...
      Parse._each(_.keys(attrs), function (attr) {
        ...
        if (_.isObject(val) && _.has(val, '__op')) {
          ...
          if (options.unset) {
            ...
            if (_.isObject(self._operations[attr]) && self._operations[attr].__op === 'Delete') {
        ...
        if (!_.isEqual(prev[attr], val) || handledOp || (_.has(now, attr) !== _.has(prev, attr))) {
    ...
    save: function (arg1, arg2, arg3) {
      ...
      var savedOperations = _.clone(model._operations); /** ignore */
  ...
  Parse.Object._findUnsavedChildren = function (object) {
    ...
    if (object instanceof Parse.Object) {
      ...
      results.push.apply(results, Parse.Object._findUnsavedChildren(object.attributes));
...
}(this)); /*global localStorage: false */
(function (root) {
  root.Parse = root.Parse || {};
  var Parse = root.Parse;
  var _ = Parse._;
...
(function (root) {
  ...
  _.extend(Parse.Collection.prototype, Parse.Events, /** @lends Parse.Collection.prototype */ {
...
}(this)); /*global _: false, document: false */
(function (root) {
  root.Parse = root.Parse || {};
  var Parse = root.Parse;
  var _ = Parse._;
  ...
  _.extend(Parse.View.prototype, Parse.Events, /** @lends Parse.View.prototype */ {
...
(function (root) {
  ...
  Parse.User = Parse.Object.extend("_User", /** @lends Parse.User.prototype */ {
    ...
    signUp: function (attrs, options) {
      ...
      if (!username || (username === "")) {
        if (options && options.error) {
          ...
          Parse.Error.OTHER_CAUSE, "Cannot sign up user with an empty name.");
      ...
      if (!password || (password === "")) {
        if (options && options.error) {
          error = new Parse.Error(
          Parse.Error.OTHER_CAUSE, "Cannot sign up user with an empty password.");
          options.error(this, error);
        }
        return false;
      }
    ...
    _saveCurrentUser: function (user) {
      ...
      Parse._getParsePath(Parse.User._CURRENT_USER_KEY), JSON.stringify(json));
    },
      ...
      var ajaxOptions = {
        ...
        success: function (response) {
          if (response) {
            ...
            error(null, new Parse.Error(Parse.Error.OBJECT_NOT_FOUND, "Object not found."));
          }
        }
      };
...
(function (root) {
  ...
  var provider = {
    ...
    restoreAuthentication: function (authData) {
      ...
      if (authData) {
        authResponse.userID = authData.id;
        authResponse.accessToken = authData.access_token;
        authResponse.expiresIn = (Parse._parseDate(authData.expiration_date).
        getTime() - (new Date()).getTime()) / 1000;
      } else {
        authResponse.userID = "deauthenticated";
        authResponse.accessToken = "deauthenticated";
  ...
  Parse.FacebookUtils = {
    ...
    init: function (options) {
      ...
      // Prevent bad use of the FB.logout function.  This can go away if
      // Facebook gives us a good way to set the authResponse
      ...
      FB.init(_.extend({
        authResponse: {
          userID: "deauthenticated",
          accessToken: "deauthenticated",
          expiresIn: null
        }
      }, options));
